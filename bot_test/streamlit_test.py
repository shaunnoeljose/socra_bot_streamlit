...
def code_analysis_agent(code: str) -> str:
    ...
    if not code or not code.strip():
        return "Code Analysis Result: No code provided for analysis. Please provide a code snippet."
...
def code_explanation_agent(concept: str) -> str:
    ...
    if not concept or not concept.strip():
        return "Explanation Result: No concept provided for explanation. Please specify a Python concept."
...
@tool
def challenge_generator_agent(topic: str = "Python Basics", difficulty: str = "beginner") -> str:
    """
...
@tool
def mcq_agent(topic: str = "Python Basics", difficulty: str = "beginner") -> str:
    """
...
@tool
def route_to_socratic_question(query: str = "") -> str:
...
@tool
def route_to_challenge_generator(topic: str = "Python Basics", difficulty: str = "beginner") -> str:
...
@tool
def route_to_mcq_generator(topic: str = "Python Basics", difficulty: str = "beginner") -> str:
...
def filter_messages(messages: List[BaseMessage]) -> List[BaseMessage]:
    ...
    filtered = []
    for msg in messages:
        if isinstance(msg, BaseMessage) and msg.content and msg.content.strip():
            filtered.append(msg)
    return filtered
...
5.  **Persistence in Learning:** Continue to ask follow-up questions on the current concept until the user explicitly states they understand or are ready to move to a new topic or challenge. Do not assume understanding; prompt them to confirm.
6.  **Interpret Tool Outputs Socratically:** If a tool provides information (e.g., Code Analysis Result, Explanation Result, Challenge Result), your task is to *process that information* and turn it into a Socratic question or guided step for the user. Do not just relay the tool's output directly.
7.  **Maintain Context:** Keep track of the current topic and sub-topic.
8.  **Be Patient and Encouraging:** Foster a positive learning environment.
9.  **Strict Output Format:** Your response MUST be a direct Socratic question. Do NOT include any "Thought:" prefix, internal monologue, conversational filler, or anything other than the question itself. Ensure the question is the ONLY content.
...
def call_supervisor(state: SocraticAgentState):
    messages = filter_messages(state["messages"][-MAX_MESSAGES_IN_CONTEXT:])
    
    # Ensure we have at least one message
    if not messages:
        return {
            "messages": [AIMessage(content="What would you like to learn about Python today?")],
    ...
    if response.tool_calls:
        ...
        elif hasattr(tool_call_item, "name"):
            tool_name = tool_call_item.name
            tool_input = tool_call_item.args if hasattr(tool_call_item, "args") else {}
        elif hasattr(tool_call_item, "function"):
            tool_name = tool_call_item.function.name
            tool_input = tool_call_item.function.arguments if isinstance(tool_call_item.function.arguments, dict) else {}
...
def socratic_question_node(state: SocraticAgentState):
    ...
    # Ensure we have at least one message
    if not messages:
        # No messages, start with the initial prompt
        return {
    ...
    # Check if the last message already generated a response
    last_user_message = messages[-1].content
    if "?" in last_user_message or "?" in state["agent_thought"]:
        # If there's an ongoing question, avoid generating another one
        return {
            "messages": [],
    ...
    try:
        ...
        state["agent_thought"] = response.get('content', "")
        return {
            "messages": [AIMessage(content=state["agent_thought"])],
            "agent_thought": state["agent_thought"]
        }
        ...
        return {
            "messages": [AIMessage(content="An error occurred while processing your request. Please try again.")],
            "agent_thought": ""
        }
...
def generate_mcq_node(state: SocraticAgentState):
    ...
    if tool_function:
        try:
            ...
            updated_state["mcq_active"] = True
            updated_state["mcq_question"] = mcq_data["question"]
            updated_state["mcq_options"] = mcq_data["options"]
            updated_state["mcq_correct_answer"] = mcq_data["correct_answer"]
        except Exception as e:
            mcq_raw_output = f"Error generating MCQ: {e}"
    
    return {"messages": [ToolMessage(content=mcq_raw_output, name=tool_name, tool_call_id=str(uuid.uuid4()))], **updated_state}
...
def reset_chat():
    st.session_state.socratic_state = SocraticAgentState(
        ...
        mcq_options=[],
        mcq_correct_answer="",
        agent_thought="",  # Ensure this is reset
        next_node="",
        tool_input={}
    )
    ...
    with chat_container:
        with st.chat_message("assistant"):
            ...
            if st.button("Submit Answer", key="submit_mcq"):
                if selected_option:
                    selected_index = mcq_opts.index(selected_option)
                    ...
                    if selected_index == correct_index:
                        ...
                        st.error(f"Incorrect. The correct answer was {correct_ans}. Let's review the concept.")
...
if user_input:
    ...
    # Run the graph
    try:
        # Create a new state dictionary for the graph execution
